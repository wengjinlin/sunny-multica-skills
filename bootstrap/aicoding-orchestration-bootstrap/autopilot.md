...
- 权限就绪门（`role_bind=pending`，仅含 auth-resource.sql 的 change）：超一个巡检周期未回执「权限已就绪」→ 评论提醒发起人在库上执行该 SQL 并到权限系统 UI 将菜单/按钮绑定到测试账号所属角色（即其给 Tester 配置 TEST_ROLE 环境变量时填的角色名；agent 无权查 env，提醒只引用变量名）。**只提醒，不阻塞任何 stage 推进**（stage 4 点火不依赖此回执，未绑定时 Tester 的菜单可见性自检会作为环境阻塞上报）。
