...
```sql
-- =====================================================================
-- {change-id} 权限资源注册（菜单树 + 接口登记 + 按钮登记，目标库 dev 172.26.58.135:1521/orcl）
...
-- 0. 前置观测（计数查应 0 行；非 0 先贴回判读。后两查为惯例观测供第 4 段对齐）
...
SELECT COUNT(*) FROM AUTH_ROLE_RESOURCE_URL WHERE C_URL LIKE '/{模块路由前缀}/%';  -- 应 0 且执行后仍 0
SELECT DISTINCT C_AREA FROM AUTH_RES_BUTTON;                                          -- 核对 C_AREA 拼写
SELECT ... FROM AUTH_RES_BUTTON b JOIN AUTH_RES_MENU m ... WHERE b.C_NAME='新增';    -- 核对 C_STOREMETHOD 承载与惯例列
SELECT C_MODNAME, C_ICON FROM AUTH_RES_MENU WHERE C_ICON IS NOT NULL AND ROWNUM <= 10;  -- 观测 C_ICON 取值惯例
...
--    放行方式：fmchen 在权限系统 UI 给真实角色绑定上述菜单+接口（人工操作，本文件永不生成授权语句）。
...
```sql
-- 预期：菜单行 N+1（含目录）；AUTH_MODULE_URL 行 M；按钮行 K；AUTH_ROLE_RESOURCE_URL 0 行
...
SELECT COUNT(*) AS cnt FROM AUTH_ROLE_RESOURCE_URL WHERE C_URL LIKE '/{模块路由前缀}/%';  -- 预期 0（UI 绑定角色后才会非 0）
